...
while(True):  
    if(playButton == 'Y' or playButton == 'y'):
        numColorDict = {1: "red", 2: "yellow", 3: "orange", 4: "green", 5: "blue"}
        while counter != 1:
            userSelection = input("Select a number between 1 and 5, press N/n to end the game: ")
            print("-----------{}".format(numColorDict.keys()))
            try:
                if int(userSelection) in numColorDict.keys():
                    print("Your selected color : {}. Let us see what computer selects the color".format(numColorDict.get(int(userSelection))))
                    computerSelection = random.randint(1,5)
                    print("Computer selected : {}".format(numColorDict.get(int(computerSelection))))
